...
class Signifyd_Connect_Model_Observer extends Varien_Object
{
    ...
    public function handleCancel($incrementId)
    {
        ...
        if ($case->isObjectNew()) {
            $this->logger->addLog("Guarantee cancel: Signifyd case for order {$incrementId} does not exist in DB");
            return;
        }

        if (in_array($case->getGuarantee(), array('N/A', 'DECLINED', 'CANCELED'))) {
            $this->logger->addLog('Guarantee cancel skipped, because case guarantee is ' . $case->getGuarantee(), $case);
            return;
        }

        $this->logger->addLog("Guarantee cancel for case " . $case->getCode(), $case);
    ...
    public function salesOrderPaymentCancel($observer)
    {
        try {
            ...
            if ($payment instanceof Mage_Sales_Model_Order_Payment) {
                ...
                $this->logger->addLog("Event salesOrderPaymentCancel has no order");
                return;
            }
        } catch (Exception $e) {
        ...
        try {
            ...
            if ($creditmemo instanceof Mage_Sales_Model_Order_Creditmemo) {
                ...
                if ($order->canCreditmemo()) {
                    $this->logger->addLog("Order {$order->getIncrementId()} still can be refunded, case will not be cancelled", $order);
                    return;
                }
